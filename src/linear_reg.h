...
typedef struct ivlf64 ivlf64;
struct ivlf64 { double min; double max; };
slr GlobalSLR = {0};
...
double SLRAverageF32(float *Data, uint64_t Count)
{
  double Sum = 0.0;
  for(uint64_t i=0;i<Count;i++)
  {
...
ivlf64 SLRDataSetRange(int32_t *Data, uint32_t Count)
{
  ivlf64 Result = {.min=F64Max, .max=F64Min};
  for(int i=0;i<Count;i++)
  {
...
ivlf64 SLRDataSetRangeF64(double *Data, uint32_t Count)
{
  ivlf64 Result = {.min=F64Max, .max=F64Min};
  for(int i=0;i<Count;i++)
  {
...
double *SLRGetMeanNormalizedCopy(int32_t *Data, uint32_t Count)
{
  ...
  ivlf64 SetRange = SLRDataSetRange(Data, Count);
  for(int i=0;i<Count;i++)
  {
...
double *SLRGetMeanNormalizedCopyF64(double *Data, uint32_t Count)
{
  double *Result = ArenaPushArray(&GlobalSLR.Arena, Count, double);
  double Avg   = SLRAverageF64(Data, Count);
  ivlf64 SetRange = SLRDataSetRangeF64(Data, Count);
  for(int i=0;i<Count;i++)
  {
    Result[i] = (double)(Data[i]-Avg)/(double)(SetRange.max-SetRange.min);
  }
...
double SLRCostF32(double w, double b, float *Features, float *Expected, uint32_t BatchCount)
{
  double Sum = 0.0;
  for(int i=0;i<BatchCount;i++)
  {
...
slr_gradient SLRGradientF32(double w, double b, float *Features, float *Expected, uint32_t BatchCount)
{
  double dJ_dW = 0.0;
  double dJ_dB = 0.0;
  for(int i=0;i<BatchCount;i++)
...
void SLRGradientDescentF32(uint32_t IterationCount, double LearnRate, double *wInitial, double *bInitial, float *Features, float *Expected, uint32_t BatchCount)
{
  //get the cost
  //compute the dererivative of the cost
  //go towared the slope of given by the finding the derivative
  ...
  for(int i=0;i<IterationCount;i++)
  {
    slr_gradient Gradient = SLRGradientF32(w, b, Features, Expected, BatchCount);
    if(isnan(b) || isnan(w)) { printf("[iter-%d]nan detected\n",i); KillProcess(); }
    w = w - LearnRate*Gradient.w;
    b = b - LearnRate*Gradient.b;
    if(IterationCount%100==0)
    {
      printf("[%.4d]cost: %.4f\n", i, SLRCostF32(w,b,Features, Expected, BatchCount));
    }
  }
  *wInitial = w;
  *bInitial = b;
...
void SLRRun(void)
{
  ...
  double w = 0.0f;
  double b = 0.0f;
  uint32_t IterationCount = 10000;
  double lr = 0.0000000000001;
  arena LoadedArena = ArenaAlloc(Megabytes(200));
  csv AreasCsv  = CSVInitFromFile("..\\data\\areas.csv", &LoadedArena);
  csv PricesCsv = CSVInitFromFile("..\\data\\prices.csv", &LoadedArena); 
  uint32_t BatchCount = Min(AreasCsv.RowCount, PricesCsv.RowCount);
  double *MeanNormArea = NULL;//SLRGetMeanNormalizedCopyF64(AreasCsv.Data, BatchCount);
  
  if(MeanNormArea == NULL) return;
  ...
  printf("learn rate:       %f\n"
         "iterations count: %lu\n", lr, IterationCount);
  printf("w: %12.4f, b: %12.4f, cost: %12.4f\n", w, b, SLRCostF32(w, b, (float *)AreasCsv.Data, (float *)PricesCsv.Data, BatchCount));
  SLRGradientDescentF32(IterationCount, lr, &w, &b, (float *)MeanNormArea, (float *)PricesCsv.Data, BatchCount);
  DBGGenPythonFile(w, b);
  printf("w: %12.4f, b: %12.4f, cost: %12.4f\n", w, b, SLRCost(w, b, SLRSampleArea, SLRSamplePrices, BatchCount));
  float *Prices = (float *)PricesCsv.Data;
  float *Areas  = (float *)AreasCsv.Data;
  for(int i=0;i<BatchCount; i++)
  {
    printf("prediction: %f vs expected: %f \n", w*Areas[i]+b, (double)Prices[i]);
  }
